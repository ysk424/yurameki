...
def _load_dll():
    ...
    try:
        chain_frame = dll.yurameki_cuda_simulate_chain_frame
    except AttributeError:
        chain_frame = None
    try:
        chain_frame_v2 = dll.yurameki_cuda_simulate_chain_frame_v2
    except AttributeError:
        chain_frame_v2 = None
    if chain_frame is not None:
        chain_frame.argtypes = [
            ctypes.c_int,
            ctypes.c_int,
            ctypes.POINTER(ctypes.c_float),
            ctypes.POINTER(ctypes.c_float),
        ...
        chain_frame.restype = ctypes.c_int
    if chain_frame_v2 is not None:
        chain_frame_v2.argtypes = [
            ctypes.c_int,
            ctypes.c_int,
            ctypes.POINTER(ctypes.c_float),
            ctypes.POINTER(ctypes.c_float),
        ...
        chain_frame_v2.restype = ctypes.c_int
...
def simulate_chain_frame_arrays(points_xyz: np.ndarray,
                                prev_roots_xyz: np.ndarray,
                                target_roots_xyz: np.ndarray,
                                segment_lengths: np.ndarray,
                                vertices: np.ndarray,
                                ...
                                collider_substeps: int,
                                max_move_m: float,
                                propagation_length_m: float,
                                style_targets_xyz: np.ndarray | None = None,
                                style_weights: np.ndarray | None = None,
                                style_strength: float = 0.0) -> tuple[np.ndarray, int]:
    """Run one V0.6 fixed-chain simulation frame inside CUDA."""
    dll = _load_dll()
    try:
        chain_frame = dll.yurameki_cuda_simulate_chain_frame
    except AttributeError as exc:
        raise RuntimeError(
            "yurameki_cuda_simulate_chain_frame is missing. "
            "Rebuild native/yurameki_cuda_collide.dll for Yurameki 0.6.0."
        ) from exc
    try:
        chain_frame_v2 = dll.yurameki_cuda_simulate_chain_frame_v2
    except AttributeError:
    ...
    if lengths.shape != (n_strands, pps - 1):
        raise ValueError("segment_lengths must be strands x (points - 1)")
    use_style = style_strength > 0.0 and style_targets_xyz is not None and style_weights is not None
    if use_style:
        if chain_frame_v2 is None:
            raise RuntimeError(
                "yurameki_cuda_simulate_chain_frame_v2 is missing. "
                "Rebuild native/yurameki_cuda_collide.dll for Yurameki 0.6.4."
            )
        style_targets = np.ascontiguousarray(style_targets_xyz, dtype=np.float32)
        weights = np.ascontiguousarray(style_weights, dtype=np.float32)
        if style_targets.shape != points.shape:
            raise ValueError("style_targets_xyz must match points_xyz")
        if weights.shape != (n_strands, pps):
            raise ValueError("style_weights must be strands x points")
    else:
        style_targets = None
        weights = None
    out_points = np.zeros_like(points)
    ...
    if use_style:
        code = chain_frame_v2(
            ...
            _as_float_ptr(lengths),
            ctypes.c_float(float(gravity_step_m)),
            ctypes.c_float(float(propagation_length_m)),
            _as_float_ptr(style_targets),
